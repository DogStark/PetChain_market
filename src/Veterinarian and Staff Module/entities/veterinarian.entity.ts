...
export class Veterinarian {
  ...
  @OneToMany(() => Credential, credential => credential.veterinarian, {
    cascade: true,
  })
  credentials!: Credential[];

  @OneToMany(() => AvailabilitySchedule, schedule => schedule.veterinarian, {
    cascade: true,
  })
  ...
  @ManyToMany(
    () => Specialization,
    specialization => specialization.veterinarians,
    { cascade: true },
  )
  @JoinTable({
  ...
  get activeCredentials(): Credential[] {
    return (
      this.credentials?.filter(
        cred => cred.isActive && new Date(cred.expirationDate) > new Date(),
      ) || []
    );
  }
  ...
  get isAvailable(): boolean {
    return (
      ...
      this.activeCredentials.length > 0
    );
  }
}
